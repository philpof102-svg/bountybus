...
const results = await pipeline(
  ...
  (f) => agent(
    ...
    + `Return at most your ${MAX} strongest findings.`,
    { label: `hunt:${f.key}`, phase: 'Hunt', schema: FINDINGS_SCHEMA, agentType: 'general-purpose', effort: 'high' }
  ).then((r) => ({ finder: f.key, dir: f.dir, reviewed: (r && r.reviewed) || '', findings: (r && r.findings) || [] })),
  (res) => parallel(
...
if (!survivors.length) log('Zero survivors is a valid, honest result on a mature target — not a failure of the pass.')
...
return {
  ...
  coverage: FINDERS.map((f, i) => ({ finder: f.key, reviewed: (results[i] && results[i].reviewed) || '(see journal)' })),
  counts: { candidates: all.length, survivors: survivors.length, rejected: rejected.length },
}
